...
#该协程封装的是查询事务,第一个参数为sql语句,第二个为sql语句中占位符的参数列表,
# 第三个参数是要查询数据的数量
async def select(sql, args, size=None):
    log(sql, args)

    #此处的global __pool我没有搞明白,因为create_pool肯定是要在select执行之前完成的,
    # 一旦完成,__pool就成了全局变量,没有必要在此处再次声明,以下是测试
    def func():
        global y
        y = 100
    def func1():
...
# loop.run_until_complete(create_pool(host='127.0.0.1', port=3306,user='root', password='Lj18119929389',db='sakila', loop=loop))
# rs = loop.run_until_complete(select('select * from actor',None))
# #获取到了数据库返回的数据
# print("heh:%s" % rs)